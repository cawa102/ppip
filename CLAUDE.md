...
**Status: harness implemented; GPU rollout body pending.** The full harness — evaluator, candidate validation, metrics/scoring, budgets, the autoresearch search loop, ledger, and aggregation — is implemented and tested (`pyproject.toml` + modules under `src/`). Plan Tasks 2–6 are done. The remaining GPU-dependent work is `OpenVLARolloutBackend.run_rollouts` (`src/evaluator/openvla_backend.py`) and `src/rendering/` (visual-prompt injection into LIBERO scenes), tracked as Phase C in `~/.claude/plans/`. On the GPU host, run tests with the proven env: `~/vla-injection/.venv/bin/python -m pytest` (see the `gpu-env-vla-injection` memory). Remaining plan work: Task 1 (threat-model/lit-map polish) and Task 7 (pilot). Do not treat placeholder configs (e.g. `PLACEHOLDER_USER_TASK`) as real data.
