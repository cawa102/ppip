...
**Status: planning/scaffold.** There is no implementation yet — no `.py` files, no dependency manifest, no runnable evaluator. Directories under `src/`, plus `experiments/{results,candidates}`, currently hold README placeholders and template JSON. Implementation follows the task list in `docs/plans/2026-07-01-autoppia-vla.md` (Tasks 1–7, sequential). Do not treat placeholder configs (e.g. `PLACEHOLDER_USER_TASK`) as real data.
