...
The evaluator orchestration depends only on this Protocol, so it can be tested with
a fake backend. The real `OpenVLARolloutBackend` (which loads the model, renders the
visual prompt into the LIBERO scene, and runs the closed loop) lives behind the same
interface and is wired in through the configured GPU rollout environment.
"""
