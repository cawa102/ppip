...
> **⚠️ Every `word_gate_probe.py` command below pins `--stratify 32` (added 2026-08-17).** The specs
> always said "the same 32 stratified train frames", but the copy-paste commands carried no sampling
> flag, so the default glob swept all **1471** frames in `runs/monitor-stealth/ceiling/frames/train`:
> **~55 GPU-h per point instead of ~2 h** (46x), and λ=0/0.1/0.3 landed on a different sample than the
> λ=1.0 baseline. **`--limit` is not the fix** — it truncates a *sorted* buffer, so `--limit 32`
> returns 32 consecutive frames of `init01` (verified) and `lam1.0/README.md` warns against exactly
> that. `--stratify N` (added with the flag) reuses `objective_probe.stratified_sample` and reproduces
> the baseline's 4-frames-per-init footing. See
> `docs/research/word-gate-artifact-experiments.md` §7.
...
$PY experiments/patch_attack/word_gate_probe.py \
    --effect targeted --word please --index 0 --lam 0.0 --stratify 32 \
...
for LAM in 0.1 0.3 3 10; do
  $PY experiments/patch_attack/word_gate_probe.py \
      --effect targeted --word please --index 0 --lam "$LAM" --stratify 32 \
...
for W in carefully now zx; do
  $PY experiments/patch_attack/word_gate_probe.py \
      --effect targeted --word "$W" --index 0 --lam 1.0 --stratify 32 \
...
for I in $(seq 0 10); do
  $PY experiments/patch_attack/word_gate_probe.py \
      --effect targeted --word please --index "$I" --lam 1.0 --stratify 32 \
...
Costs below assume `--stratify 32`; without it every entry is 46x larger.*
...
$PY experiments/patch_attack/word_gate_probe.py --effect targeted --word please --index 0 --stratify 32 \
