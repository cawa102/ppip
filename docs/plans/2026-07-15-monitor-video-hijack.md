...
- [x] **Task 5: Neutral-teacher render + S0 sanity gate** — DONE 2026-07-16 (CPU + **GPU verified**). `experiments/patch_attack/monitor_attack.py`: `neutral_texture` (uniform mid-gray 128 — benign, distinct from the black/white calibration probes), `summarize_s0`/`S0Report` (seed-exclusion policy: a seed whose TARGET fails with the neutral monitor is flagged out, never silently used; `all_pass` is the gate), `teacher_tokens` (GPU seam: OpenVLA's real-path target-conditioned tokens on a FRESH post-upload neutral render — Task-4 invariant; reuses `adaptive_attack._real_tokens` so get_action preprocessing never drifts), `s0_sanity` (GPU seam: per-seed deployment-scene episode with neutral monitor + TARGET instruction, latches the fixed TARGET goal → `S0Report`). Tests `tests/patch_attack/test_monitor_attack.py`: 3 CPU-pure (neutral texture; exclusion policy both branches) + **3 GPU-verified** — `teacher_tokens` deterministic & in-vocab (2 sequential 7B loads OK); `s0_sanity` structure smoke (seed 0, max_steps=8); neutral-vs-attack frames differ ONLY inside the monitor (`outside_mask_delta ≤ OUTSIDE_TOLERANCE=3.0` — the monitor's ~1px AA edge, same render slack GATE A validated; my first `== 0.0` was over-strict → fixed the test, not the code). **Deviations:** (1) tests live in a new `test_monitor_attack.py` (1:1 with the module) not the Task-4 contract file, for cohesion; (2) S0 uses the USER (deployment) scene with the TARGET instruction/goal — matches `adaptive_attack`'s reachable-target setup and the plan's "same geometry as deployment". `pyproject`: added the GPU-env-only search-side modules (`vla_diff`, `patch_config`, `hijack_backend`, `adaptive_attack`, `monitor_upload_probe`) to the mypy ignore-missing override (same rationale as the torch/mujoco entries). ruff + mypy `--strict` clean; CPU 158 passed / 11 skipped. **Note:** the machinery is verified, but the actual S0 *gate run* (seeds 0–4, full horizon, recording exclusions) is a GPU experiment still to run — it validates the teacher before Task 6/7.
...
- [x] **Task 8: Open-loop replay (Stage 2) + controls + margin report (GATE B)** — DONE 2026-07-16. **GATE B = FAIL at seed 0 → boundary result (do NOT build Phase 1).** The physically-realizable monitor does NOT hijack OpenVLA: the Stage-1 oracle (upper bound) degenerated to the neutral monitor (`select_texture` never found an attack texture beating neutral), never advancing past phase 0; Stage-2 replay is **byte-identical to the blank & time-scrambled controls** (`min_target_dist=0.35425` for all three), `phase_margin=0`, `hijack_beats_controls=False`. **Mechanism (instrumented):** the per-step token-match trace is mean **6.88 / mostly 7/7** — USER- and TARGET-instructed policies emit the *same* action tokens on the rollout frames, so OpenVLA's action is scene- not language-driven, neutral is already at ~minimal CE, and the monitor-confined attack (eps 0.15, ~16% of frame, through the render reality-gap) is too weak to flip the greedy action; the honest target is not instruction-reachable (S0 False at 130 **and** 280 steps — not a horizon artifact). Contrast: `adaptive_attack` hijacked seed 0 only via a full-frame camera-buffer write (L∞→0.6). Headline: `runs/monitor-hijack/README.md`; data `runs/monitor-hijack/seed0/gate_b_result.json`; driver `experiments/patch_attack/run_gate_b.py`. **Caveat:** one seed (mechanism systematic → replication expected; a seeds-0–4 sweep would confirm). `experiments/patch_attack/monitor_replay.py`: `time_indexed_texture` (`video[t]`, a function of the step index ALONE — no state-conditioned selection is expressible), `scramble_video` (deterministic time-scramble control), `ReplayResult`/`margin_report` (the GATE-B panel; the hijack claim is a MARGIN — `hijack_beats_controls` requires the attack to targeted-succeed while neither blank nor scrambled does). GPU seams `run_replay`/`run_control` (blank/scrambled) reuse `setup_deployment_episode`. Tests `tests/patch_attack/test_monitor_replay.py`: 4 CPU-pure + **1 GPU smoke** (2-frame replay + both controls → full margin panel). ruff + mypy `--strict` clean. **The real GATE-B decision** is produced by `experiments/patch_attack/run_gate_b.py` (S0 → Stage-1 oracle recording `texture_0..T` → Stage-2 replay + blank/scrambled controls → margin), launched on seed 0 (130 steps) now that GPU 1 freed. Result → `runs/monitor-hijack/seed0/gate_b_result.json`.
...
- [x] **Task 9: Living-docs update (mandatory, same change)** — DONE 2026-07-16. `runs/monitor-hijack/README.md` (GATE-B headline + mechanism + caveats), `docs/research/research-log.md` (2026-07-16 entry + superseding START HERE), `CLAUDE.md` status line, this plan (Tasks 5–9 ticked), and the `monitor-hijack-phase0` memory all record the GATE-B FAIL boundary result, GATE A/B decisions, and the reproduce command.
