...
- `experiments/patch_attack/two_branch_loss.py` — WP3 objective `two_branch_loss(...)`; CE mirrors
  `monitor_patch_attack.py:244`. *(WP2/WP3 live in their own modules, not `word_gate.py`: cohesion,
...
- `experiments/patch_attack/word_gate.py` (`GateSetup`/`resolve_gate_setup`, pure) +
  `experiments/patch_attack/monitor_patch_attack.py` (`run_confined_episode` edit) — WP7 additive
